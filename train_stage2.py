...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,scale=1,output_dir=None,mode="train",lrmodel_path=None, sr_version="stable",orthogonal=False):
    pass

def training_mip(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,scale=1,output_dir=None,mode="train",lrmodel_path=None, sr_version="stable",orthogonal=False,num_view=0,use_2d=None,no_pixel=False,lambda_fea=1,lambda_lr=1,lambda_hr=1,col_mip=False):
    ...
    gaussians_lr = GaussianModel_mip(dataset.sh_degree)
    gaussians_lr.load_ply(os.path.join(lrmodel_path))
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if int(viewpoint_cam.up_scale)>2: # LR from GS [4,8]
            with torch.no_grad():
                viewpoint_cam_lr = copy.deepcopy(viewpoint_cam)
                viewpoint_cam_lr.image_height=viewpoint_cam_lr.image_height//2
                viewpoint_cam_lr.image_width=viewpoint_cam_lr.image_width//2
                render_lr = render_mip(viewpoint_cam_lr, gaussians_lr, pipe, background, kernel_size=kernel_size_used)["render"]
                down_scale=2
        else:
            render_lr=viewpoint_cam.original_image.cuda()
            down_scale=2 if float(viewpoint_cam.up_scale)>1 else 1
   
        image = F.avg_pool2d(image, kernel_size=(down_scale, down_scale))
        Ll1_lr = l2_loss(image, render_lr)
        loss_lr = (1.0 - opt.lambda_dssim) * Ll1_lr + opt.lambda_dssim * (1.0 - ssim(image, render_lr))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lrmodel_path", type=str, default=None)
    ...
    if args.mip:
        training_mip(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from,args.scale,args.out_dir,args.mode,args.lrmodel_path, args.sr_version,args.orthogonal,args.num_view,args.use_2d,args.no_pixel,args.lambda_fea,args.lambda_lr,args.lambda_hr,args.col_mip)
    else:
        training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from,args.scale,args.out_dir,args.mode,args.lrmodel_path, args.sr_version,args.orthogonal,args.num_view)
