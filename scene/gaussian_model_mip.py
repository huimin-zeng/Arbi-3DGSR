...
#
from scene.torch_quantile import torch_quantile
import torch
import numpy as np
...
from .StableSR import get_stablesr,get_stablesr_vq

class GaussianModel_mip:
    ...
    def __init__(self, sh_degree : int, with_stablesr=False, with_stablesr_vq=[]):
        ...
        if with_stablesr:
            self.stablesr=get_stablesr()
            for param in self.stablesr.parameters():
                param.requires_grad = False

        if len(with_stablesr_vq)>0:
            self.stablesr_vq=get_stablesr_vq(with_stablesr_vq)
            for i in with_stablesr_vq:
                for param in self.stablesr_vq[i].parameters():
                    param.requires_grad = False
    ...
    def get_2D_filter(self):
        ...
        # kernel_size=0.1 -> distance / focal_length * (0.2 ** 0.5)
        return torch.square(self.filter_3D) / 2
    ...
    def compute_3D_filter(self, cameras):
        ...
        for camera in cameras:
            ...
            if focal_length < camera.focal_y:
                focal_length = camera.focal_y
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        # Q = torch_quantile(grads_abs.reshape(-1), 1 - ratio)
