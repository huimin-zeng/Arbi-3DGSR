...
from scene.StableSR.ldm.modules.diffusionmodules.model import Encoder, Decoder, Decoder_Mix
from scene.StableSR.ldm.modules.distributions.distributions import DiagonalGaussianDistribution
...
from scene.StableSR.basicsr.data.degradations import random_add_gaussian_noise_pt, random_add_poisson_noise_pt, random_add_speckle_noise_pt, random_add_saltpepper_noise_pt
import random
...
class AutoencoderKLResi(pl.LightningModule):
    ...
    @apply_forward_hook
    def encode_fea(self, x):
        return self.encoder(x, return_fea=True)
    ...
    def encode(self, x, return_moments=False):
        ...
        # posterior = h
        if return_moments:
            return posterior, enc_fea, moments
        else:
