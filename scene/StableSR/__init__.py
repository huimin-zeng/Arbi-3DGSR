...
def get_stablesr():
    ...
    # configs = [OmegaConf.load("scene/StableSR/bsr_sr.yaml")]
    ...
    # pl_sd = torch.load("../sr_bsr_model.ckpt", map_location="cpu")
...
def get_stablesr_vq(model_list=["diff","vq"]):
    return_model_list={}
    parser = argparse.ArgumentParser()
    ...
    if "vq" in model_list:
        if len(model_list)==1: #only vq, use vqgan encoder
            vqgan_config = OmegaConf.load("scene/StableSR/LD_first_stage_models_vq_f8.yaml")
            vq_model = load_model_from_config(vqgan_config, "../vq_f8_n256_model.ckpt")
        else:
        ...
        return_model_list["vq"]=vq_model.cuda()

    if "diff" in model_list:
        seed_everything(opt.seed)
        ...
        model.register_schedule(given_betas=None, beta_schedule="linear", timesteps=1000, linear_start=0.00085, linear_end=0.0120, cosine_s=8e-3)
        ...
        print(trainable_params)
        print(untrainable_params)
        return_model_list["diff"]=model.cuda()
